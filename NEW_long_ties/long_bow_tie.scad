...
// shifts the upper dove to right
_offset = 0;

module bow_tie_master(length){
    dovetail_3d(male_dove_max_width,
        male_dove_min_width,
        male_dove_depth,
        length);
}
...
module long_bow_tie_unscaled(length) {
    ...
    module _half() {
        ...
        difference() {
            ...
            translate([13,5,0])
            rotate([90,0,25]) endcyl();

            translate([-13,5,0])
            rotate([90,0,-25])
            endcyl();

            translate([13,-length-5,0])
            rotate([90,0,-25])
            endcyl();
...
module long_bow_tie(length=length,
                    scale_x=scale_x(),
                    scale_z=scale_z(),
                    middle=false) {
...
module split(length) {
    ...
    polygon(points=[[0,-1.4], [1,-1.4], [2,-5], [-2,-5], [-1,-1.4]]);
}
...
module long_bow_tie_split(length=length,
    ...
    if (!middle) {
        translate([0,0,scale_z*male_dove_depth])
        _do_split();
    } else {
        _do_split();
    }
...
module long_bow_tie_half(length=length,
                         scale_x=scale_x(),
                         scale_z=scale_z(),
                         middle=true) {
    rotate([0,-90,0])
...
module long_bow_tie_half_split(length=length,
                               scale_x=scale_x(),
                               scale_z=scale_z()) {
    union() {
        intersection() {
            long_bow_tie_split(length,
                ...
                middle=true);
            translate([0,0,-6/2])
            cube([10,length,6],center=true);
        }
        cube([scale_x*male_dove_min_width,length,1.6],center=true);

        translate([((scale_x*male_dove_min_width)/2)/2,0,4/2])
        cube([(scale_x*male_dove_min_width)/2,length,4],center=true);
