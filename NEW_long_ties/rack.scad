...
module _herringbone_rack(length) {
    herringbone_rack(l=length,
                     h=rack_h,
                     w=10,
                     tooth_size=rack_tooth,
                     CA=rack_teeth_angle);
}

module tie_taken(){
    translate([-(total_length+20)/2,0,tail_depth-0.01])
    rotate([90,0,90])
    male_dovetail(height=total_length+20, bridge_extra=0.2);

    translate([-(total_length+20)/2,-9,9.4])
    rotate([90,180,90])
        ...
        union(){
            intersection(){
                ...
                difference(){
                    union(){
                        translate([0,0,6])
                        rounded_cube(total_length-slop,
                                     40, 20, diameter,
                                     center=true, $fn=20);

                        translate([-total_length/2-head_offset+slop,6,-15])
                    ...
                    // back chamfer
                    union(){
                        translate([total_length/2-head_offset-slop/2,6,-15])
                        rotate([0,0,-rack_teeth_angle])
                        cube([10,10,30]);
