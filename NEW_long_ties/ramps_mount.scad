...
module ramps_enclosure() {
    ...
    union() {
        difference() {
            ...
            difference() {
                rounded_cube_side(w-2*wall,l-2*wall,h,6-2*wall,center=true);
                ...
                // cable arches
                translate([w/2-3/2-wall,-17.1,-h/2+40/2])
                cube([3,2,40],center=true);

                translate([w/2-39/2+3/2,-17.1,-h/2+40/2])
                cube([3,2,40],center=true);
...
module ramps_enclosure_lid() {
    ...
    difference() {
        union() {
            ...
            difference() {
                translate([0,0,wall])
                    rounded_cube_side(w-wall_offset*2,
                                      l-wall_offset*2,
                                      3,
                                      6-wall_offset*2,
                                      center=true);
                translate([0,0,4/2])
                    rounded_cube_side(w-10,l-15,4,3,center=true);
            }
...
module raspberry_pi_3b_mount() {
    ...
    %translate([-29.5,45,0.6])
    rotate([0,0,-90])
    import("../../_downloaded/Raspberry_pi_3_reference/Raspberry_Pi_3_Light_Version.STL");
    ...
    difference() {
        ...
        translate([0,0,2.5])
        cube([raspberry_pi_3b_w,raspi_mount_l+1,h], center=true);

        for(i = [0:7]) {
            translate([0,i*7-raspi_mount_l/3.4,-h/2])
            _stripe();
        }
        translate([-raspi_mount_w/2-1,27,8/2-1.6])
        cube([9.8,9,4]);

        translate([-raspi_mount_w/2-1,2.4,8/2-1.6])
        cube([10,16,4]);

        translate([-raspi_mount_w/2-1,-11.2,8/2+2])
        rotate([0,90,0]) cylinder(d=8,h=5,$fn=20);
    }

    translate([raspberry_pi_3b_w/2, raspi_mount_l/2-3.75,h/2-1])
    _notch(w=1.5,l=7.5);

    translate([raspberry_pi_3b_w/2, -raspi_mount_l/2+21,h/2-1])
    _notch(w=1.5);

    translate([-raspberry_pi_3b_w/2, raspi_mount_l/2-3.5,h/2-1])
    _notch(w=1.5,l=7);

    translate([-raspberry_pi_3b_w/2, -raspi_mount_l/2+22,h/2-1])
    _notch(w=1.5,l=10);

    translate([-raspi_mount_w/2+0.01,raspi_mount_l/2-15,-h/2])
    long_tie_half(20);
...
module atx_connector_mount() {
    ...
    union() {
        difference() {
            ...
            translate([0,2,14/2+wall])
            cube([inner_w, l+1, h], center=true);

            translate([0,0,14/2+wall+1.5])
        ...
        translate([outer_w/2-0.01,-l/2+15,0])
        rotate([0,0,180]) long_tie_half(20);
...
module fan_mount_60mm() {
    ...
    difference() {
        ...
        rotate([0,0,180])
        linear_extrude(0.6) text(align="center",size=8, "RAMPS 1.4");
    }
}
