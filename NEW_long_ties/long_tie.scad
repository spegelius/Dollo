...
module long_tie(length=length) {
    translate([0,0,scaled_male_dove_depth()]) rotate([0,180,0]) difference(){
        ...
        translate([-5,-length/2-1,scaled_male_dove_depth()]) cube([10,length+2,male_dove_depth]);
    }
}
...
module long_tie_split(length=length) {
    union() {
        difference() {
            ...
            translate([0,0, -3.5]) cube([10,length+1,7], center=true);
        }
        translate([0,0, 0.8]) cube([4,length,1.6], center=true);
    }
}
...
module long_tie_half(length=length) {
    rotate([0,-90,0]) intersection() {
        long_tie(length);
        translate([0,-length/2,0]) cube([5,length,5]);
    }
}
