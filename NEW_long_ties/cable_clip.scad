...
_cable_shroud_motor_clip_body(h=12);
...
module _shroud_holder(h=25, d=13.5, edge_h=1, gap=9) {
    ...
    difference() {
        ...
        cylinder(d1=d - 2, d2=d, h=0.7, $fn=30);

        translate([0, 0, edge_h + 0.7])
        cylinder(d=d, h=h + 1 - edge_h, $fn=30);

        translate([-gap/2, 0, 0])
        cube([gap, outer_d, h + 1]);
    }
}
...
module cable_shroud_frame_mount() {
    ...
    union() {
        difference() {
            ...
            translate([8.01, 20/2, 0])
            rotate([0, 0, 90])
            male_dovetail(h + 1.1);
        }

        translate([-(12 + 2.4)/2 + 0.01, 10, 0])
        rotate([0, 0, 90])
        _shroud_holder();
    }
}
...
module cable_shroud_frame_clip() {
    ...
    union() {
        ...
        translate([-(12 + 2.4)/2 + 0.01, 10, 0])
        rotate([0, 0, 90])
        _shroud_holder(edge_h=-1, gap=11);
    }
}
...
module cable_shroud_frame_clip2() {
    ...
    intersection() {
        difference() {
            ...
            translate([w/2, -16.5, h/2])
            chamfered_cube(
                w + 50, 32, h - 8, 14, center=true
            );

            translate([w/2, 8.01, 0])
            rotate([0, 0, 180])
            male_dovetail(h + 1.1);

            translate([w/2, -30/2 + 2.8, h/2])
            difference() {
                cylinder(
                    d=30, h=3, $fn=50, center=true
                );

                cylinder(
                    d=27, h=4, $fn=50, center=true
                );
            }
        }
        translate([w/2, 0, h/2])
        cube([w + 2, 100, h], center=true);
    }
}
...
module _cable_shroud_motor_clip_body(h=12) {
    ...
    difference() {
        union() {
            ...
            translate([
                0,
                -(motor_side_length + 8)/2 - 20/2 + 13,
                0
            ])
            chamfered_cube_side(
                16.5 + 5, 18, h, 1, center=true
            );
        }
        difference() {
            chamfered_cube_side(
            ...
            for(i = [0:3]) {
                rotate([0, 0, 360/4*i]) {
                    translate([
                        motor_side_length/2 + 3/2 - 0.5,
                        4, 0
                    ])
                    cylinder(
                        d=3, h=h, center=true, $fn=20
                    );
...
module cable_shroud_motor_clip() {
    ...
    difference() {
        union() {
            ...
            translate([
                -motor_side_length/2 - 3,
                -motor_side_length/2 + 4
            ])
            rotate([90, 0, 0])
            cylinder(
                d=5, h=15, center=true, $fn=40
            );
        }
        translate([
            -motor_side_length/2 - 3,
...
module frame_spacer() {
    difference() {
        union() {
            ...
            translate([0, 15/2 + 0.2, 30/2])
            rotate([-90, 0, 0])
            long_bow_tie_split(30, middle=true);
        }
        translate([0, -15/2, 0])
        male_dovetail(31);
    }
