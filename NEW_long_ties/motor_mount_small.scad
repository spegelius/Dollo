...
module y_mount_added(motor_side){    
    ...
    difference() {
        ...
        cylinder(d=7.25, h=2.5, $fn=30);
    }
}

module bolt_hole() {
    hull() {
        cylinder(d=bolt_hole_dia, h=70, $fn=30);
...
module tapered_bolt_head_hole() {
    hull() {
        cylinder(
            d1=bolt_hole_dia, d2=6.5, h=3, $fn=30
        );
...
module y_mount_taken(bridges, motor_side){
	...
	rotate([0, 0, 45]) {
        ...
        translate([
            5.65 - 21, 5.65 - 21, bolt_hole_h()
        ])
        bolt_hole();

		translate([
            5.65 + 31-21, 5.65 - 21, bolt_hole_h()
        ])
        bolt_hole();

		translate([
            5.65 - 21, 5.65 + 31 - 21, bolt_hole_h()
        ])
        bolt_hole();
        ...
        if (motor_side) {
            translate([
                5.65 + 31 - 21, 5.65 + 31 - 21, -10
            ])
            bolt_hole();

            translate([
                5.65 + 31 - 21, 5.65 + 31 - 21, 0.5
            ])
        ...
        if (motor_side) {
            ...
            hull() {
                ...
                cylinder(
                    d=motor_center_hole - 8, h=18
                );

                translate([
                    hole_length, hole_length, 0
                ]) {
                    cylinder(
                        d=motor_center_hole,
                        h=4, $fn=30
