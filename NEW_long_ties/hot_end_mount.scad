...
module _fan_duct_nozzle(nozzle_offset, nozzle_width) {
    ...
    module nozzle_main() {
        translate([
            -nozzle_width/2 + 1 + nozzle_offset,
            0, 0
        ])
        hollow_tube();
        ...
        difference() {
            ...
            translate([
                -nozzle_width/2 + nozzle_offset + 1,
                0, -1
            ])
            cylinder(d=12, h=2.1);
    ...
    difference() {
        ...
        // air cutouts
        translate([
            nozzle_width/2 + nozzle_offset - 4,
            20/2 + 1, -4
        ])
        rotate([0, 60, 0])
...
module _fan_duct_radial(
    nozzle_offset, nozzle_width, hot_end,
    supports=false
) {
        ...
        difference() {
            union() {
                hull() {
                    ...
                    translate([
                        nozzle_offset, 7, 13/2 + 1
                    ])
                    rotate([90 + 25, 0, 0])
                    hull() {
                        translate([
                            -nozzle_width/2 + 1, 0, 0
                        ])
                        cylinder(d=13, h=1);
...
module fan_duct_radial_e3dvolcano(
