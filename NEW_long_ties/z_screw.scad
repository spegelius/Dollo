...
module debug_screw_housing() {
    ...
    intersection() {
        union() {
            ...
            translate([0, 0, screw_housing_height])
            rotate([180, 0, -1/3*360])
            screw_housing_top();

            translate([side_roller_offset, 0, z])
            rotate([rise_angle, 0, 0])
            translate([0, 0, -side_roller_height/2 - 1])
            side_roller();

            rotate([0, 0, side_roller_angle])
            translate([side_roller_offset, 0, z])
            rotate([rise_angle, 0, 0])
            translate([0, 0, -side_roller_height/2])
            side_roller();

            rotate([0, 0, side_roller_angle*2])
            translate([side_roller_offset, 0, z])
            rotate([rise_angle, 0, 0])
            translate([0, 0, -side_roller_height/2 + 1])
            side_roller();
            ...
            color("black") {
                translate([side_roller_offset, 0, z])
                rotate([rise_angle, 0, 0])
                translate([0, 0, -side_roller_height/2 - 2])
                side_roller_axle_washer();
            ...
            translate([
                0, 0,
                -(side_roller_height + 8 - slop)/2 - 1.1
            ])
            side_roller_axle();
        }
        translate([-1, -1, -50])
...
module debug_screw_center() {
    intersection() {
        union() {
            ...
            translate([0, 0, 85])
            z_screw(120, fast_render=true);

            translate([0, 0, 85 + 120])
            z_screw(120, fast_render=true);

            translate([0, 0, 85 + 120*2])
            z_screw(120, fast_render=true);

            translate([0, 0, 33])
            z_screw_center_coupler();

            translate([0, 0, 86 + 120/2])
            z_screw_center();

            translate([0, 0, 86 + 120/2 + 120])
            z_screw_center();
        }
        translate([-25, 0, 0])
        cube([50, 50, 1200]);
...
module debug_gears() {
    ...
    translate([side_roller_offset, 0, 0])
    rotate([rise_angle, 0, 0])
    _roller_gear();
}
...
module debug_screw_roller() {
    intersection() {
        union() {
            ...
            translate([side_roller_offset, 0, -z_step/2])
            rotate([rise_angle, 0, 0])
            rotate([0, 0, 9])
            side_roller();
        }
...
module side_roller() {
    ...
    function y_step(step) = 
        total_r/cos(asin((x_step*step)/total_r)) - total_r;
    echo(y_step(3));
    
    steps = [2, 1, 0, 1, 2];

    d_z = z_step + side_roller_d;
    ...
    difference() {
        union() {
            for (i = [0:4]) {
                intersection() {
                    hull()
                    translate([0, 0, i*z_step + z_step/2])
                    cube_donut(
                        10 + 2*y_step(steps[i]),
                        cube_w(z_step), angle=360,
                        rotation=45, $fn=100
                    );

                    cylinder(
                        d=d_z + 2*y_step(steps[i])-0.3,
                        h=side_roller_height
                    );
                }
                //echo(2*y_step(steps[i]));
            }
            translate([0, 0, z_step/2])
            _roller_gear(side_roller_height - z_step);

            cylinder(
                d=side_roller_d + 0.3,
                h=side_roller_height, $fn=100
            );
        }

        cylinder(
            d=side_roller_axle_d + slop/2,
            h=6*z_step + 1, $fn=60
        );
    }
}
...
module _screw_housing(frame_width=22) {
    ...
    module screw_hole() {
        ...
        union() {
            cylinder(
                d=side_roller_d + z_step + 1,
                h=side_roller_height + 2.2
            );

            translate([0, 0, -3])
            rotate([0, 0, -1])
            _axle_end(side_roller_axle_d);
        }
    }
    ...
    difference() {
        cylinder(
            d=z_screw_d + frame_width,
            h=screw_housing_height/2, $fn=100
        );

        cylinder(
            d=z_screw_d + 1, h=screw_housing_height/2 + 1
        );

        for(i = [0:side_rollers - 1]) {
            rotate([0, 0, i*side_roller_angle])
            translate([side_roller_offset, 0, z])
            rotate([rise_angle, 0, 0]) {
                translate([0, 0, 2 + i*_z_step])
                screw_hole();
            }
        }
    }
...
module screw_housing_bottom(
    frame_width=22, render_threads=true
) {
    ...
    difference() {
        ...
        for(i = [0:side_rollers - 1]) {
            rotate([0, 0, i*side_roller_angle + 45])
            translate([side_roller_offset, 0, z - 3])
            cylinder(d1=4, d2=5, h=3);

            if (render_threads) {
                rotate([0, 0, i*side_roller_angle - 45])
                translate([side_roller_offset + 1, 0, 1])
                v_screw(
                    screw_d=screw_housing_thread_d,
                    h=screw_housing_height,
                    pitch=1.8,
                    direction=0,
                    steps=30
                );
...
module screw_housing_top(frame_width=22) {
    ...
    difference() {
        ...
        for(i = [0:side_rollers - 1]) {
            rotate([0, 0, i*side_roller_angle + 45])
            translate([side_roller_offset + 1, 0, -0.1]) {
                cylinder(
                    d=screw_housing_thread_d,
                    h=screw_housing_height
                );
                ...
                cylinder(
                    d1=screw_housing_thread_d + 3,
                    d2=screw_housing_thread_d, h=2
                );
            }
        }
        rotate([0, 0, -360/side_rollers])
        translate([(z_screw_d + frame_width)/2, 0, 0])
        rotate([0, 0, 45])
        cube([1, 1, 50]);
    }
    for(i = [0:side_rollers - 1]) {
        rotate([0, 0, i*side_roller_angle - 45])
        translate([side_roller_offset, 0, z])
        cylinder(d1=5 - slop, d2=4 - slop, h=3);
    }
}
...
module screw_housing_bolt() {
    difference() {
        union() {
            difference () {
                cylinder(
                    d1=screw_housing_thread_d + 3 - slop,
                    d2=screw_housing_thread_d - 2*slop, h=2
                );
                cube([
                    1.5, screw_housing_thread_d + 4, 3
                ], center=true);
            }
            translate([0, 0, 2])
            cylinder(
                d=screw_housing_thread_d - 2*slop,
                h=screw_housing_height/2 - 3
            );

            translate([0, 0, screw_housing_height/2 - 1])
            v_screw(
                screw_d=screw_housing_bolt_d,
                h=screw_housing_height/2 - 2,
                pitch=1.8,
                steps=40,
                direction=0
            );
...
module screw_housing_bolt_side() {
    ...
    difference() {
        ...
        intersection() {
            ...
            cube([
                13, 50, screw_housing_thread_d + 2
            ], center=true);
        }
        block();
...
module _axle_end(d) {
    ...
    hull() {
        translate([0, 0, 1/2])
        cube([cube_d, cube_d, 1], center=true);
...
module side_roller_axle_washer() {
    difference() {
        ...
        cylinder(
            d=side_roller_axle_d, h=screw_housing_height/2
        );
    }
}
...
module _coupler_screw(fast_render=false) {
    union() {
        intersection() {
            translate([0, 0, -2*coupler_h])
            z_screw(coupler_h*3, fast_render=fast_render);

            cylinder(d=z_screw_d, h=coupler_h);
        }
        translate([0, 0, -slop])
        pyramid(z_screw_center_w, cap=1.5);
    }
}
...
module z_screw_motor_coupler(fast_render=false) {
    ...
    difference() {
        union() {
            cylinder(
                d1=motor_center_hole - 1,
                d2=z_screw_d - 0.5, h=4, $fn=50
            );

            translate([0, 0, 4])
            cylinder(d=z_screw_d - 0.5, h=31 - 4, $fn=50);

            translate([0, 0, 31])
            _coupler_screw(fast_render);
        }
        translate([0, 0, -3])
        motor_shaft(h=34, extra_slop=slop);

        translate([0, 0, 8])
        rotate([-90, 0, 0])
        cylinder(d=bolt_d, h=20);

        translate([0, 8.5, 8])
        rotate([-90, 0, 0])
        cylinder(d1=bolt_d, d2=8, h=5);

        hull() {
            translate([-m3_nut_side/2, 4.5, 0])
            cube([m3_nut_side, m3_nut_height + slop, 1]);

            translate([0, 4.5, 8])
            rotate([-90, 30, 0])
            M3_nut(m3_nut_height + slop, cone=false);
        }
    }
}
...
module z_screw_motor_flex_coupler(fast_render=false) {
    difference() {
        union() {
            ...
            translate([0, 0, 12.5])
            cylinder(d1=15, d2=z_screw_d - 1, h=2.5);

            translate([0, 0, 15])
            cylinder(d=z_screw_d - 4, h=1.5);
